...
- (id)init
{
    ...
    yaml_emitter_set_output(opaque_emitter, (yaml_write_handler_t *) CFDataAppendBytes, (__bridge void *) buffer);
...
- (int)_writeItem:(id)item toDocument:(yaml_document_t *)doc
{
    ...
    if ([item respondsToSelector:@selector(keyEnumerator)]) {
        ...
        nodeID =
                yaml_document_add_scalar(doc, (yaml_char_t *) YAML_DEFAULT_SCALAR_TAG, (yaml_char_t *) [[item description] UTF8String], (int) [[item description] length], YAML_ANY_SCALAR_STYLE);
    }
    return nodeID;
}
