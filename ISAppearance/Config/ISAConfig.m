...
@interface ISAConfig() <ISYAMLParserTagResolver>
...
+ (ISAConfig *)sharedInstance {
    static ISAConfig *_instance = nil;
...
- (BOOL)processAppearance {
    ...
    [[ISAppearance sharedInstance] setAppearanceReady:YES];
    return YES;
}
...
- (void)processUIAppearance:(NSDictionary *)definition baseKeys:(NSArray *)baseKeys {
    ...
    [definition enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        ...
        if ([key isKindOfClass:[NSString class]]) {
            key = [(NSString*)key componentsSeparatedByString:@":"];
        }

        if ([key isKindOfClass:[NSArray class]]) {
            if ([(NSArray*)key count] == 1) {
                key = key[0];
            }
        }
                ...
                if ([(id) cl conformsToProtocol:@protocol(UIAppearance)]) {
                    ...
                    switch (classes.count) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            appearanceProxy = [cl appearanceWhenContainedIn:classes[0], classes[1], classes[2], classes[3], classes[4], classes[5], nil];
                            break;
                        default:
...
- (BOOL)checkStyleConformance:(NSArray *)selectors passedSelectors:(NSArray **)pPassedSelectors {
    ...
    return [[ISAppearance sharedInstance] checkStyleConformance:selectors passedSelectors:pPassedSelectors];
}
...
- (void)addParams:(NSArray *)params toSelector:(NSArray *)components {
    ...
    [[ISAppearance sharedInstance] addParams:params forClass:baseClass toSelector:userComponents];
}
...
- (void)updateAppearanceRegisteredObjects {
    for (id object in [_registeredObjects copy]) {
        if ([object respondsToSelector:@selector(isaClass)]) {
            [[ISAppearance sharedInstance] applyAppearanceTo:object usingClassesString:[object isaClass]];
        }
        else {
            [[ISAppearance sharedInstance] applyAppearanceTo:object usingClassesString:nil];
        }
    }
    if (_monitoring) {
        [CATransaction flush];
